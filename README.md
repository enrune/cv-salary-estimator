...
### Omezení (otevřeně)
- Synthetic data jsou odhad, ne autoritativní zdroj
- Scraped data jsou snapshot k datu spuštění (`python -m src.scrape` pro refresh)
- Cca 30 % inzerátů na jobs.cz nemá uvedenou mzdu — ty se ignorují
...
```
jobhunt/
├── data/                   # synthetic + scraped salary data
├── samples/                # ukázková CV (senior dev, pokladní, právník)
├── src/                    # 12 modulů: ingest, parse, score, salary, explain, validate, ...
├── app.py                  # Streamlit UI
├── run.py                  # CLI
└── requirements.txt
```

LLM: **OpenRouter** → default `google/gemini-2.5-flash`. Lze přepnout v `.env` (Claude, GPT, DeepSeek…). Typický run: ~4500 tokenů, ~$0.0007, ~10 s.
